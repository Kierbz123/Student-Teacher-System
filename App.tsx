      ...
      s.interventionLogs.forEach(log => {
        if (log.scheduledDate) {
          ...
          if (diffHrs > 0 && diffHrs <= 1) {
             newNotifications.push(`Upcoming (1h): Meeting with ${s.firstName} ${s.lastName} at ${sched.toLocaleTimeString()}`);
          }
          // 1 day advance notification (approx 23-25 hours)
          else if (diffHrs > 23 && diffHrs <= 25) {
             newNotifications.push(`Tomorrow: Meeting with ${s.firstName} ${s.lastName}`);
          }
        }
      });
    });
